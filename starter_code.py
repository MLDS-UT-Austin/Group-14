...
# Load the data
file_path = r"data/batting_season_summary.csv"
print(file_path)
...
data["ewma_hits"] = data.groupby("Name")["H"].transform(
    lambda x: x.ewm(alpha=alpha_value).mean()
)

# Select the last EWMA value as the next year's forecast
data["Expected Number of Hits Next Season"] = data.groupby("Name")[
    "ewma_hits"
].transform("last")
...
submission["Bid Amount($)"] = (
    submission["Expected Number of Hits Next Season"] / total_forecasted_hits
) * 200
